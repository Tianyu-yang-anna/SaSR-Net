...
def dataloader_benchmark(dataset: Dataset, max_batch_size: int=64, bs_step: int=1, max_num_workers: int=32, nw_step: int=1, iters: Optional[int]=20, *argsv, **kwargv):
    benchmarks: OrderedDict = OrderedDict()
    for bs in range(16, max_batch_size + 1, bs_step):
        for nw in range(2, max_num_workers + 1, nw_step):
            ...
            for i, _ in enumerate(dataloader):
                if iter is not None and i >= iters:
                    break 
...
def main():
    print(
        dataloader_benchmark(AVQA_dataset(label="./data/json/avqa-train.json", audio_dir="./data/feats/vggish", video_res14x14_dir="./data/feats/res18_14x14",
                                    transform=transforms.Compose([ToTensor()]), mode_flag='train'), bs_step=8, nw_step=2, iters=50)
    ) 
